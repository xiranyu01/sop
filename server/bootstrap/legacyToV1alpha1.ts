...
export type ConvertedFixtureResources = {
  customers: Customer[];
  materials: Material[];
...
export type LegacyConversion = { resources: ConvertedFixtureResources; report: ConversionReport };

function emptyConvertedFixtureResources(): ConvertedFixtureResources {
  return {
    customers: [],
    materials: [],
    robotModels: [],
...
function fingerprintRecord(source: unknown, converted: unknown): string {
  return stableHash(stableJson({ source, converted }));
}

function convertedSemanticDigest(resources: ConvertedFixtureResources): string {
  return stableHash(stableJson(resources));
}

function convertedCardinalities(resources: ConvertedFixtureResources): Record<string, number> {
  return Object.fromEntries(Object.entries(resources).map(([key, values]) => [key, values.length]));
}
...
function selectFrozenGlobalFields(
  ...
  fields: ConvertedFixtureResources['globalFields'],
  issues: ConversionIssue[],
  owner: string,
): ConvertedFixtureResources['globalFields'] {
...
function collectAttachments(data: AppData, issues: ConversionIssue[]): Map<string, Attachment> {
...
function toTaskSpec(version: SubsceneVersion, materialNames: Map<string, string>, attachmentNames: Map<string, string>, issues: ConversionIssue[], owner: string) {
...
export function convertLegacyToV1alpha1(input: unknown, sourceFingerprint = stableHash(stableJson(input))): LegacyConversion {
  ...
  resources.attachments = [...attachments.values()].sort((a, b) => stableCompare(a.name, b.name));
  ...
  for (const item of data.customers) {
    ...
    resources.customers.push(canonical); recordFingerprints[`customer:${item.id}`] = fingerprintRecord(item, canonical);
  }
  for (const item of data.materials) {
    ...
    resources.materials.push(canonical); recordFingerprints[`material:${item.id}`] = fingerprintRecord(item, canonical);
  }
  for (const item of data.scenes) {
    ...
    resources.scenes.push(canonical); recordFingerprints[`scene:${item.id}`] = fingerprintRecord({ id: item.id, name: item.name, description: item.description }, canonical);
  }
  for (const item of data.globalFields) {
    ...
    resources.globalFields.push(canonical); recordFingerprints[`globalField:${item.id}`] = fingerprintRecord(item, canonical);
  }
  for (const item of data.materialStateRules) {
    ...
    resources.materialStateRules.push(canonical); recordFingerprints[`materialStateRule:${item.id}`] = fingerprintRecord(item, canonical);
  }
  ...
  for (const item of data.robotModels) {
    ...
    resources.robotModels.push(canonical); resources.robotModelRevisions.push(create(RobotModelRevisionSchema, {
      name: revision,
  ...
  for (const scene of data.scenes) for (const subscene of scene.subscenes) {
    ...
    subscene.versions.forEach((version, index) => {
      ...
      const frozen = create(FrozenDependencyContextSchema, {
        ...
        scenes: resources.scenes.filter((candidate) => candidate.name === snapshotMessage.scene),
        globalFields: frozenGlobalFields,
        materialStateRules: resources.materialStateRules.filter((rule) => embeddedRuleNames.has(rule.name)),
        attachments: resources.attachments.filter((attachment) => attachmentSet.has(attachment.name)),
      });
    ...
    if (current?.snapshot) resources.taskSops.push(current.snapshot);
    resources.taskSopRevisions.push(...revisions);
  }
  ...
  for (const item of data.requirements) {
    ...
    const revisions = item.versions.map((version, versionIndex) => {
      ...
      const canonical = create(RequirementSchema, {
        name, uid: deterministicUid('requirement', item.id), sourceId: item.id, displayName: version.title, lifecycle: importedState.lifecycle, attachments: attachmentList, currentRevision: revName, updateTime: timestamp(version.updatedAt), candidateVersionSequence: version.status === 'draft' ? BigInt(versionIndex + 1) : undefined, candidateVersionLabel: version.status === 'draft' ? version.version : undefined, candidateSourceVersionId: version.status === 'draft' ? version.versionId : undefined,
        spec: {
      ...
      const frozen = create(FrozenDependencyContextSchema, {
        customers: resources.customers.filter((customer) => customer.name === customerName),
        globalFields: selectFrozenGlobalFields(requirementVocabularyReferences(version), resources.globalFields, issues, owner),
        attachments: resources.attachments.filter((attachment) => requirementAttachmentSet.has(attachment.name)),
      });
    ...
    const current = revisions[revisions.length - 1]; if (current?.snapshot) resources.requirements.push(current.snapshot);
    resources.requirementRevisions.push(...revisions);
  }
