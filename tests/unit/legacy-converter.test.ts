...
describe('legacy v1alpha1 converter', () => {
  ...
  it('maps the complete fixture deterministically without losing order, presence, or legacy-only values', async () => {
    const legacy = structuredClone(fixtureData);
    const first = convertLegacyToV1alpha1(legacy);
    const second = convertLegacyToV1alpha1(structuredClone(legacy));

    expect(first.report).toEqual(second.report);
    expect(stableJson(first.resources)).toBe(stableJson(second.resources));
    expect(first.report.issues).toEqual([]);

    const microwave = first.resources.taskSopRevisions.find((revision) =>
    ...
    const task = first.resources.taskSopRevisions.find((revision) => revision.versionLabel === '0.0.2' && revision.snapshot?.displayName === '洗漱台整理');
    ...
    const requirementWithZeroTarget = first.resources.requirementRevisions
  ...
  it('keeps source-position rule IDs, fractional duration precision, and indirect frozen attachments', async () => {
    ...
    const { resources: snapshot, report } = convertLegacyToV1alpha1(legacy);
